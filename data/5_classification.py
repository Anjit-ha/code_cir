...
with open("task1/step4_features.jsonl", "r") as f:
    for line in f:
...
num_samples = len(X)
if num_samples < 2:
    print(f"Note: Only {num_samples} sample(s) available. Using full dataset for training and evaluation.")
    X_train, X_test, y_train, y_test = X, X, y, y
else:
    split_idx = int(num_samples * 0.7)  # 前 70%
    X_train = X[:split_idx] if split_idx > 0 else X
    y_train = y[:split_idx] if split_idx > 0 else y
    X_test = X[split_idx:] if split_idx < num_samples else X
    y_test = y[split_idx:] if split_idx < num_samples else y
...
try:
    y_prob = clf.predict_proba(X_test)[:, 1] if len(np.unique(y)) > 1 else np.zeros_like(y_test, dtype=float)
...
print(f"AUC: {auc:.4f}" if auc > 0 else "AUC: N/A (insufficient unique classes)")
